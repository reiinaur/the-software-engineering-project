...
export default class shopScene extends Phaser.Scene {
  ...
  _draw() {
    ...
    Object.entries(TABS_DEF).forEach(([cat, def]) => {
      ...
      if (isActive) {
        ...
        // ADJUSTMENT HERE: Draw a fixed 50px wide line perfectly centered under the active tab label
  ...
  _drawCard(item, cx, cy) {
    ...
    this._track(this.add.text(cx - CARD_W / 2 + 16, textY, item.label, {
      fontFamily: "Arial", fontSize: FONT.sm, color: colours.dark, fontStyle: "bold"
    }));
    if (!isFree) {
      this._track(this.add.text(cx + CARD_W / 2 - 16, textY, `$${item.cost}`, {
        fontFamily: "'Roboto Mono', monospace", fontSize: FONT.xs, color: colours.muted
      }).setOrigin(1, 0));
    }
    this._track(this.add.text(cx - CARD_W / 2 + 16, textY + 38, item.desc, {
      fontFamily: "Arial", fontSize: "18px", color: colours.muted
    }));
    ...
    if (isEquipped) {
      this._track(this.add.text(cx, btnY, "✓   EQUIPPED", {
        fontFamily: "Arial", fontSize: FONT.xs, color: colours.success, fontStyle: "bold"
      }).setOrigin(0.5));
